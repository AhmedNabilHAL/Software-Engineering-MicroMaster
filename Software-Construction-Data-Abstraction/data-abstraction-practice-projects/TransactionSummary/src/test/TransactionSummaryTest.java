...
public class TransactionSummaryTest {
    ...
    public void testAverageTransactionCost() {
        ...
        assertEquals(testSummary.averageTransactionCost(), ((double)10 + 20 + 40 + 150 + 11 + 40 + 5)/7, 0.05);
    }

    @Test
    public void testAverageTransactionCostEmpty(){
        TransactionSummary oneSummary = new TransactionSummary("Edsger Dijkstra");
        assertEquals(oneSummary.averageTransactionCost(), 0, 0.05);
    }
    ...
    public void testspecificTypeAverage() {
        assertEquals(testSummary.specificTypeAverage(FOOD), ((double)11+5+20)/3, 0.05);
        assertEquals( testSummary.specificTypeAverage(EDUCATION), 150, 0.05);
    }

    @Test
    public void testspecificTypeAverageEmpty(){
        TransactionSummary oneSummary = new TransactionSummary("Edsger Dijkstra");
        Transaction testTransaction = new Transaction("Food", "June 12", 5, FOOD);
        oneSummary.addTransaction(testTransaction);
        assertEquals(oneSummary.specificTypeAverage(EDUCATION), 0, 0.05);
    }
}